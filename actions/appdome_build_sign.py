...
def main():
    ...
    if sign_option == 'SIGN_ON_APPDOME':
        ...
        cmd = f"appdome_virtual_env/bin/python3 appdome/appdome-api-python/appdome_api.py -key {appdome_api_key} --app {app_file} " \
              ...
              f"--certificate_output {output_path}/certificate.pdf {keystore_key_pass} " \
        ...
        if args.keystore_alias:
            cmd_aux += ["--keystore_alias", args.keystore_alias]

        print(f"running command: {' '.join(cmd_aux)}", flush=True)
        subprocess.run(cmd_aux, env=new_env, check=True, text=True)
